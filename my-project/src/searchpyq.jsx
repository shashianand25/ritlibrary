...
function getFileLeafName(file, currentSubjectCode = '') {
  ...
  const codeIndex = parts.findIndex((p) => p.toLowerCase() === currentSubjectCode.toLowerCase());
  return codeIndex >= 0
    ? parts[codeIndex + 2] || parts[parts.length - 1]
...
export default function SearchPYQ() {
        ...
        if (vId && loadedData.length > 0) {
          ...
          if (targetFile) {
            setPreviewFile({
              ...
              view: targetFile.view || targetFile.name,
            });
            window.history.replaceState({ deepLink: true }, '', `?v=${targetFile.id}`);
          }
        }
      } catch (e) {
        console.error('Failed to load GitHub database', e);
      } finally {
  ...
  useEffect(() => {
    if (form.year === '1st Year' && form.semester && form.branch) {
      const isCS = form.branch === 'cs',
        is1st = form.semester === '1st Sem';
  ...
  const handleChange = (e) => {
    ...
    if (name === '') {
      setForm({ year: value, branch: '', semester: '', subject: '', subSubject: '' });
    ...
    if (name === 'branch') {
      setForm((p) => ({ ...p, branch: value, subject: '', subSubject: '' }));
      setShowElective(false);
      return;
    }
    if (name === 'subject') {
    ...
    try {
      ...
      const pyqResults = allData.filter((file) => {
        ...
        const searchable = normalizeSearch(
          `${file.name} ${file.view || ''} ${file.folderName || ''}`
        );
  ...
  const groupedNotes = useMemo(() => {
    ...
    notesFiles.forEach((file) => {
      ...
      const generic = [
        ...
        'queston bank',
        'question bank',
        (currentSubjectCode || '').toLowerCase(),
      ];
      if (!folder || generic.includes(folder.toLowerCase())) {
        const nl = (file.view || '').toLowerCase();
        if (/\bunit[-\s]?\d+/i.test(nl) || nl.includes('unit') || nl.includes('module'))
          folder = 'Units';
        else if (nl.includes('tutorial') || nl.includes('tut') || nl.includes('exercise'))
          folder = 'Tutorials';
        else if (nl.includes('book') || nl.includes('reference')) folder = 'Books';
        else if (nl.includes('lab') || nl.includes('manual') || nl.includes('experiment'))
          folder = 'Lab Manuals';
        else if (nl.includes('question') || nl.includes('paper') || nl.includes('qb'))
      ...
      const sectionPrefix =
        structuredSection && structuredSection.toLowerCase() !== 'gen'
          ? `${structuredSection.toUpperCase()} · `
          : '';
      grouped[folder].push({
        ...file,
  ...
  const groupedPDFs = useMemo(() => {
    ...
    return Object.entries(
      pdfFiles.reduce((acc, f) => {
        const group = isAllSubjectsPyq(f)
          ? ALL_SUBJECTS_PYQ_LABEL
          : f.folderName || f.year || 'PYQ';
        const section = getFileSection(f, currentSubjectCode);
        const sectionPrefix =
          section && section.toLowerCase() !== 'gen' ? `${section.toUpperCase()} · ` : '';
          ...
          >
            ...
            >
              ...
              <div style={{ textAlign: 'center' }}>
                ...
                >
                  {examTypesList.map((type) => (
                    <PillBtn
                      key={type}
                      active={examTypes.includes(type)}
                      colors={colors}
                      onClick={() =>
                        setExamTypes((prev) => {
                          const n = prev.includes(type)
                ...
                {searchMode === 'guided' ? (
                  ...
                  >
                    ...
                    >
                      ...
                      {['1st Sem', '2nd Sem', '3rd Sem', '4th Sem', '5th Sem', '6th Sem'].map(
                        (s) => (
                      ...
                      <div style={{ position: 'relative' }}>
                        <select
                          ...
                          style={{
                            width: '100%',
                            appearance: 'none',
                            cursor: !form.year ? 'not-allowed' : 'pointer',
                        ...
                        >
                          ...
                          {form.year &&
                            branchGroups[form.year]?.map(({ label, value }) => (
              ...
              <motion.button
                ...
                whileHover={{
                  scale: isDataLoading || showSyncBtn ? 1 : 1.02,
                  boxShadow:
                    isDataLoading || showSyncBtn ? 'none' : `0 12px 28px ${colors.primary}55`,
                }}
                ...
                style={{
                  ...
                  gap: 8,
                  boxShadow:
                    isDataLoading || showSyncBtn ? 'none' : `0 6px 20px ${colors.primary}44`,
              ...
              >
                {isDataLoading || showSyncBtn ? (
                  <>
                    ...
                    {isDataLoading ? 'Connecting...' : 'Updating Cache...'}
                  </>
                ) : (
                  <>
              ...
              <AnimatePresence mode="wait">
                {showSyncRight && !submitted ? (
                  ...
                  >
                    ...
                    <div>
                      <h3 style={{ fontSize: 20, fontWeight: 800, margin: '0 0 8px 0' }}>
                        Syncing Database
                      </h3>
                      <p style={{ fontSize: 14, opacity: 0.6, margin: 0 }}>
                        Waking up the server & updating files...
                      </p>
                    </div>
                  </motion.div>
                ) : submitted ? (
                      ...
                      {showResultsLoading || isSearching ? (
                        ...
                        >
                          ...
                          <p style={{ fontWeight: 600, fontSize: 15 }}>
                            {showResultsLoading ? 'Loading Resources...' : 'Searching...'}
                          </p>
                        </div>
                      ) : activeTab === 'notes' ? (
                  ...
                  >
                    ...
                    <h3 style={{ fontSize: 26, fontWeight: 800, margin: 0 }}>
                    ...
                    >
                      ...
                      <span style={{ color: colors.primary, fontWeight: 700 }}>Branch</span> to
                      access Notes and Previous Year Question Papers.
                    </p>
...
function EmptyState({ icon: _Icon, label, colors }) {
  const Icon = _Icon;
  return (
    <motion.div
      initial={{ opacity: 0 }}
